...
export function deleteBook({commit}, id) {
...
export function updateBook({commit}, book) {
  const id = book.id
  console.log(book);
  console.log(book.image instanceof File);
  if (book.image instanceof File) {
    const form = new FormData();
  ...
  return axiosClient.post(`/books/${id}`, book)
}
